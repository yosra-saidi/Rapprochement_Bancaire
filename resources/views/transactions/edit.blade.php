...
@section('content')
    <div class="content">
        <div class="form-container">
            <h1>Editer la transaction {{ $transaction_bancaire->id }}</h1>
            <form action="{{ route('transactions.update', $transaction_bancaire->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="form-group">
                    <label for="montant">Montant :</label>
                    <input type="text" name="montant" id="montant" value="{{ $transaction_bancaire->montant }}" class="form-control" required>
                </div>
                <div class="form-group">
                    <label for="date_transaction">Date de Transaction :</label>
                    <input type="date" name="date_transaction" id="date_transaction" value="{{ $transaction_bancaire->date_transaction }}" class="form-control" required>
                </div>
                <div class="form-group">
                    <label for="payeur">Payeur :</label>
                    <select name="payeur" id="payeur" class="form-control" required>
                        @foreach($clients as $client)
                            <option value="{{ $client->id }}" {{ $transaction_bancaire->payeur == $client->id ? 'selected' : '' }}>
                                {{ $client->id}}
                            </option>
                        @endforeach
                    </select>
